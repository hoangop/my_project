...
#3c
# Step 6: Split data into training and test sets for task 3c
# Split cfat into train_fat (125 observations) and test_fat (the remainder)
...
# print("\nDataset sizes:")
# print(f"Training set (train_fat) size: {train_fat.shape}")
# print(f"Test set (test_fat) size: {test_fat.shape}")

# # Print indices of the datasets to compare against expected results
# print(f"Indices of training set (train_fat): {train_fat.index.to_list()}")
# print(f"Indices of test set (test_fat): {test_fat.index.to_list()}")


#3d
# Step 7: Forward Stepwise Selection for k=1 (Task 3d)
...
for p in allowed_factors:
    ...
    print(f"Model with {p}: R-squared = {model.rsquared:.4f}")
    if model.rsquared > best[1]:
        best = [p, model.rsquared]

# print('\nBest predictor for k=1 is:', best)

# # Save best model with 1 predictor
train_bmi1 = smf.ols(formula='Density~' + best[0], data=train_fat).fit()
print("\nSummary of model train_bmi1:")
print(train_bmi1.summary())

#3e
# Step 8: Forward Stepwise Selection for k=2 (Task 3e)
# Keep the best predictor from k=1
...
for p3 in remaining_factors_k3:
    ...
    print(f"Model with {best_k2_predictors[0]}, {best_k2_predictors[1]} and {p3}: Adjusted R-squared = {model_k3.rsquared_adj:.4f}")
    if model_k3.rsquared_adj > best_k3_combo[1]:
        best_k3_combo = [p3, model_k3.rsquared_adj]

print(f"\nBest third predictor for k=3 is: {best_k3_combo[0]}")
print(f"Best combination for k=3 is: {best_k2_predictors[0]}, {best_k2_predictors[1]} and {best_k3_combo[0]}")

# Save best model with 3 predictors
...
print("\nSummary of model train_bmi3:")
print(train_bmi3.summary())

#3g and 3h
# Step 10: Forward Stepwise Selection for k=4 (Task 3g)
# Keep the best predictors from k=3
...
for p4 in remaining_factors_k4:
    ...
    print(f"Model with {best_k3_predictors[0]}, {best_k3_predictors[1]}, {best_k3_predictors[2]} and {p4}: Adjusted R-squared = {model_k4.rsquared_adj:.4f}")
    if model_k4.rsquared_adj > best_k4_combo[1]:
        best_k4_combo = [p4, model_k4.rsquared_adj]

print(f"\nBest fourth predictor for k=4 is: {best_k4_combo[0]}")
print(f"Best combination for k=4 is: {best_k3_predictors[0]}, {best_k3_predictors[1]}, {best_k3_predictors[2]} and {best_k4_combo[0]}")

# Save best model with 4 predictors
...
print("\nSummary of model train_bmi4:")
print(train_bmi4.summary())

# Step 11: Forward Stepwise Selection for k=5 (Task 3h)
# Keep the best predictors from k=4
...
print(f"\nBest fifth predictor for k=5 is: {best_k5_combo[0]}")
print(f"Best combination for k=5 is: {best_k4_predictors[0]}, {best_k4_predictors[1]}, {best_k4_predictors[2]}, {best_k4_predictors[3]} and {best_k5_combo[0]}")

# Save best model with 5 predictors
...
print("\nSummary of model train_bmi5:")
print(train_bmi5.summary())

# Step 12: Plot Adjusted R-squared (Task 3i)
